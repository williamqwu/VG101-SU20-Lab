...
Home::Home(Point pt1, double width, double height, double owidth) {
    ...
    p2= {p.x+w/2,p.y+w/2};
    paint(&r,&g,&b);
    sh[0]=new Rectangle(p1,p2,r,g,b);
    p1= {p.x-o,p.y-w/2};
    p2= {p.x+o,p.y};
    paint(&r,&g,&b);
    sh[1]=new Rectangle(p1,p2,r,g,b);
    p1= {p.x-2*o,p.y+o};
    p2= {p.x-o,p.y+2*o};
    paint(&r,&g,&b);
    sh[2]=new Rectangle(p1,p2,r,g,b);
    p1= {p.x+w/2-2*o,p.y+o};
    p2= {p.x+w/2-o,p.y+2*o};
    paint(&r,&g,&b);
    sh[3]=new Rectangle(p1,p2,r,g,b);
    p1= {p.x,p.y+h-w/2};
    p2= {p.x-w/2,p.y+w/2};
    p3= {p.x+w/2,p.y+w/2};
    paint(&r,&g,&b);
    sh[4]=new Triangle(p1,p2,p3,r,g,b);
}
Home::~Home() {
